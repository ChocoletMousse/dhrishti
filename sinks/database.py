...
class FirestoreReddit:
    ...
    def get_submissions(self, limit: int = 10) -> list:
        ...
        documents = (
            self.db.collection(self.subreddit_ref)
            .order_by("landing_timestamp", direction=Query.DESCENDING)
            .limit(limit)
            .stream()
        )
    ...
    def get_comments(self, limit: int = 10) -> list:
        ...
        documents = (
            self.db.collection(self.comments_ref)
            .order_by("landing_timestamp", direction=Query.DESCENDING)
            .limit(limit)
            .stream()
        )
    ...
    def get_responses(self, limit: int = 5) -> list:
        ...
        logging.info(f"getting {limit} comments")
        documents = (
            self.db.collection(self.responses_ref)
            .order_by("landing_timestamp", direction=Query.DESCENDING)
            .limit(limit)
            .stream()
        )
...
class BigQueryReddit:
    ...
    def write_to_table(self, documents: list):
        ...
        bq_errors = self.db.insert_rows_json(self.table_name, json_rows=documents)
        if len(bq_errors) > 0:
